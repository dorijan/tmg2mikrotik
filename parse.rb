...
my_file = File.open("Computers.rsc", 'w')
doc.xpath('fpc4:Root').xpath('fpc4:Arrays').xpath('fpc4:Array').xpath('fpc4:RuleElements').xpath('fpc4:Computers').xpath('fpc4:Computer').each do |doo|
		puts doo.xpath('fpc4:Name').text
		
		#doo.xpath('fpc4:IPAddress')[0].xpath('fpc4:Str').each do |dio|
			my_file.puts 'do { /ip firewall address-list add list=Computers address="'+doo.xpath('fpc4:IPAddress').text+'" comment="'+doo.xpath('fpc4:Name').text+'"} on-error={}' rescue puts "Error in: "+dio.text
			my_file.write "\r\n"
		#end
